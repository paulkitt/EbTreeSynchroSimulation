...
object Constant {
  ...
  final val GRAPHIC_ACTIVE = false
  final val LOG = false
...
object SimulationMaster extends App {
  ...
  log.info("[SimulationMaster] select Simulation: \n" +
    ...
    "--------------------Dynamische Tests------------------------------\n"+
    "(7): Zellreplikation unter Last\n" +
    "(8): Bestimmen der benötigten Synchronisationsresourcen im laufenden Betrieb\n" )
      ...
      elemNum.foreach(num => {
        ...
        1.to(repetition).toList.foreach(x => {
          ...
          Constant.SYNCHRO_EVENTS.foreach(event => addEvent(event, {
            val eventNum: Double = EventLogging.getEvent(event).getOrElse(0).toDouble; if (eventNum == 0) 0 else eventNum / response._3
          }))
        })
...
//              Thread.sleep(50)
            ...
            Constant.SYNCHRO_EVENTS.foreach(event => addEvent(event, {
              val eventNum: Double = EventLogging.getEvent(event).getOrElse(0).toDouble; if (eventNum == 0 || response._3 == 0) 0 else eventNum / response._3
            }))
          })
